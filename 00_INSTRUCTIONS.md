...
- Dark DGS theme, palette derived from the logo (navy `#0C3060` · blue `#185490` · orange `#E48424`). Full token table in `design/design_system.md`.
- The logo appears **only** in the top bar and footer, on a white plate — never bare on a dark surface, never in the hero (D17).
- Pages under `docs/` are generated by `tools/build_site.py`; never hand-edit a page.
...
**Site map (`docs/`):** `index.html` · `roadmap.html` · `sessions.html` · `session-NN/index.html` × 18 · `labs/index.html` · `projects/index.html` · `resources/tools.html` · `resources/practice.html` · `downloads/` · `.nojekyll`

**Naming rule (D16):** the site is organised by **session** — `docs/session-NN/index.html` for NN = 01–18, listed on `sessions.html`, with the 6 modules kept as the grouping on `roadmap.html`. Repo packages use the same numbers (`packages/session-NN/`). What still never appears in a student-facing file: the 2-month calendar, the session length, and any wording that explains content placement by lack of time.
...
- [ ] No mention of the 2-month calendar, session length, or time-driven placement in any student-facing file
